...
class ServerCrypto extends Crypto {
  ...
  encryptPacket (message) {
    if (message.messageType == EMsg.ServerHello || message.messageType == EMsg.LoginFailed) {
      ...
      let nonce = new Nonce({ clientKey: this.clientKey, serverKey: this.serverKey, nonce: this.decryptNonce });
      let toEncrypt = Buffer.concat([this.encryptNonce.getBuffer(), this.client.getSharedKey(), Buffer.from(message.decrypted)]);

      let cipherText = this.encrypt(toEncrypt, nonce);

      this.setSharedKey(this.client.getSharedKey());
