...
const options = require('./src/util/usage').options;
const settings = require('./settings.json');

require('console-stamp')(console, 'yyyy-mm-dd HH:MM:ss');

const PacketReceiver = require('./src/packet_receiver');
const ClientCrypto = require('./src/client/crypto');
const ServerCrypto = require('./src/server/crypto');
const Definitions = require('./src/definitions');
const EMsg = require('./src/messages');

const definitions = new Definitions(options);
const clients = {};
...
if (options.replay) {
  ...
  server.on('connection', function (socket) {
    let gameserver = new net.Socket();
    socket.key = socket.remoteAddress + ':' + socket.remotePort;
    clients[socket.key] = socket;

    let clientPacketReceiver = new PacketReceiver();
    let serverPacketReceiver = new PacketReceiver();

    let clientCrypto = new ClientCrypto(settings);
    let serverCrypto = new ServerCrypto(settings);
